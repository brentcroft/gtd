 ...
 */
public class W3cHTMLSelectElementGuiObject< T extends HTMLSelectElement > extends W3cHTMLElementGuiObject< T >
		implements GuiObject.Index, GuiObject.Click
{
	public W3cHTMLSelectElementGuiObject( T go, Gob parent, GuiObjectConsultant< T > guiObjectConsultant,
	...
	public void click()
	{
		// NB: unlike HTMLInputElement, HTMLSelectElement doesn't expose a click()
		// method
	...
	public void buildSelectModel( Element element, Map< String, Object > options )
	{
		...
		if ( !GuiObject.isShallow( options ) )
		{
			...
			int itemCount = DataLimit.MAX_LIST_DEPTH.getMin( list.getLength(), options );
			...
			for ( int i = 0, n = itemCount; i < n; i++ )
			{
				...
				maybeSetElementAttribute( cellElement, "text",
						format( "[%s]%s", maybeGetValueType( value ), maybeConvertValue( value.getValue() ) ) );
			}
		}
	}
