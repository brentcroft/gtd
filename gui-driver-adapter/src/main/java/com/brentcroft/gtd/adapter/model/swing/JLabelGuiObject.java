...
{
	public JLabelGuiObject( T go, Gob parent, GuiObjectConsultant< T > guiObjectConsultant,
	...
	enum Attr implements AttrSpec< JLabel >
	{
		...
		ICON( "icon", go -> ofNullable( go.getIcon() ).map( JComponentGuiObject::getPathyModel ).orElse( null ) ),

		FOR_HASH( HASH_FOR_ATTRIBUTE, go -> ofNullable( go.getLabelFor() )
				.map( component -> Integer.toString( component.hashCode() ) ).orElse( null ) );
