...
public class TabPaneDetacher
{
    ...
    private static final double NEW_MODELLER_HEIGHT = 520;
    private static final double NEW_MODELLER_WIDTH = 760;
    ...
    public void openTabInStage( final Tab tab )
    {
        ...
        double prefWidth = NEW_MODELLER_WIDTH;
        double prefHeight = NEW_MODELLER_HEIGHT;

        if ( content instanceof Region )
        {
            prefWidth = ( ( Region ) content ).getPrefWidth();
            prefHeight = ( ( Region ) content ).getPrefHeight();
        }
        ...
        {
            throw new IllegalArgumentException( "Can not detach Tab '" + tab.getText() + "': not an instance of Parent: " + content.getClass() );
        }

        tab.setContent( null );
        final Scene scene = new Scene( ( Parent ) content, prefWidth, prefHeight );
